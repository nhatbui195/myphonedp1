...
export default function OrderManagement() {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      const res = await api.get(`/api/admin/donhang`);
      setOrders(Array.isArray(res.data) ? res.data : []);

      const req = await api.get(`/api/admin/yeucauhuy`);
  ...
  const confirmOrder = async (id) => {
    ...
    try {
      await api.put(`/api/admin/xacnhan-don/${id}`);
      await load();
  ...
  const confirmCancel = async (id) => {
    ...
    try {
      await api.put(`/api/admin/xacnhan-huy/${id}`);
      await load();
  ...
  const rejectCancel = async (id) => {
    ...
    try {
      await api.put(`/api/admin/tuchoi-huy/${id}`);
      await load();
  ...
  const updateStatusManual = async (id) => {
    ...
    try {
      await api.put(`/api/admin/capnhat-trangthai/${id}`, { TrangThai });
      await load();
  ...
  const removeOrder = async (id) => {
    ...
    try {
      await api.delete(`/api/admin/xoadonhang/${id}`);
      await load();
  ...
  const approvePayment = async (orderId) => {
    ...
    try {
      await api.put(`/api/admin/donhang/approve-payment/${orderId}`);
      await load();
  ...
  const refundPayment = async (id) => {
    ...
    try {
      await api.put(`/api/admin/donhang/refund-payment/${id}`);
      await load();
  ...
  const markPicking = async (id) => {
    ...
    try {
      await api.put(`/api/admin/donhang/mark-picking/${id}`);
      await load();
  ...
  const markShipping = async (id) => {
    withLoading("Đang chuyển trạng thái VC...");
    try {
      await api.put(`/api/admin/donhang/mark-shipping/${id}`);
      await load();
  ...
  const markDelivered = async (id) => {
    withLoading("Đang chuyển trạng thái VC...");
    try {
      await api.put(`/api/admin/donhang/mark-delivered/${id}`);
      await load();
  ...
  const cancelShipping = async (id) => {
    ...
    try {
      await api.put(`/api/admin/donhang/cancel-shipping/${id}`);
      await load();
